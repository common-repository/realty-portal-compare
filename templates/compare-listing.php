...
if ( isset( $_SESSION[ 'rp_compare_properties' ] ) && ! empty( $_SESSION[ 'rp_compare_properties' ] ) ) :
	...
	if ( ! empty( $list_property ) ) :
		...
		$class_compare      = ( ( $total_list_compare === 4 ) ? 'rp-md-5' : ( $total_list_compare === 3 ? 'rp-md-3' : ( $total_list_compare === 2 ? 'rp-md-4' : 'rp-md-6' ) ) );
                ...
                <div class="rp-compare-wrap">
					...
					<?php while ( $compare_query->have_posts() ): $compare_query->the_post();
                        ...
                        <div class="rp-compare-item <?php echo esc_attr( $class_compare ) ?>">
							...
							<?php echo get_the_term_list( $property->ID, apply_filters( 'rp_property_listing_type', 'listing_type' ), '<div class="rp-compare-item-detail"><label>' . esc_html__( 'Type:', 'realty-portal-compare' ) . '</label><span>', ', ', '</span></div>' ); ?>
							...
							<?php echo get_the_term_list( $property->ID, apply_filters( 'rp_property_listing_offers', 'listing_offers'), '<div class="rp-compare-item-detail"><label>' . esc_html__( 'Offers:', 'realty-portal-compare' ) . '</label><span>', ', ', '</span></div>' ); ?>
